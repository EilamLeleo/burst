...
numApical = np.linspace(0,100,num=41)#50,num=21)#
...
spks2 = [[0 for i in range(len(ApicalBasalInterval))] for j in range(len(treeTime))] 
frqs2 = [[0 for i in range(len(ApicalBasalInterval))] for j in range(len(treeTime))]
#trc = spks
#tme = trc
...
for ApiBasInd in ApicalBasalInterval:
    print "Running for dt: %s [ms]" % (ApiBasInd)#interval: %s [ms]" % (int(ApiBasInd))
...
#        numA = int(totalS*0.4)
        spks2[j][i],frqs2[j][i] = runSim(L5PC,ApiBasInd,treeT,65,35,10,medSegment,20)
        j = j+1
    j = 0
    i = i+1
